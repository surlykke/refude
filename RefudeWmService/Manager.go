...
import (
	"errors"
	"github.com/surlykke/RefudeServices/RefudeWmService/xlib"
...
func MakeManager() Manager {
	var m Manager
	m.in = xlib.MakeConnection()
	m.out = xlib.MakeConnection()
	m.mappedWindows = []*Window{}
	return m
}
...
func (m *Manager) updateWindows() {
	if wIds, err := m.in.GetUint32s(0, NET_CLIENT_LIST_STACKING); err != nil {
		...
		var resourcesToMap = make([]resource.Resource, len(wIds))
		for i, wId := range wIds {
			...
			resourcesToMap[i] = newMappedWindows[i]
		}

		m.mappedWindows = newMappedWindows
		resourceCollection.RemoveAndMap([]resource.StandardizedPath{"/window"}, resourcesToMap)
	}
}
