...
func locateIconInTheme(iconName string, size uint32, theme *IconTheme) string {
	...
	for _, basedir := range basedirs {
		if existsAndIsDir(basedir + "/" + theme.ThemeId) {
			for _, iconDir := range theme.Dirs {
				if existsAndIsDir(basedir + "/" + theme.ThemeId + "/" + iconDir.Path) {
					for _, extension := range []string{".png", ".svg"} { // TODO deal with xpm
						var tentativeFilename = basedir + "/" + theme.ThemeId + "/" + iconDir.Path + "/" + iconName + extension
